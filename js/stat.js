...
var USER_BG_COLOR = 'rgba(0, 0, 255, 1)';
...
var renderBar = function (ctx, color, name, time, index, barHeight, x) {
  ...
  ctx.fillStyle = color;
  ctx.fillRect(positionX, CLOUD_HEIGHT - barHeight - STEP * 4, HISTOGRAM_COLUMN_WIDTH, barHeight);
  ctx.fillStyle = CLOUD_SHADOW_COLOR;
  ctx.fillText(name, positionX, CLOUD_HEIGHT - STEP * 2);
  ctx.fillText(time, positionX, CLOUD_HEIGHT - barHeight - STEP * 8);
};
